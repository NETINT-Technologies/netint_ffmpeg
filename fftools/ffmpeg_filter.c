                       ...
                       AVFrame *frame)
{
    ...
    while (fgp->nb_outputs_done < fg->nb_outputs) {
        ...
        if (ret == AVERROR(EAGAIN)) {
            ...
            //NETINT if all inputs have ended return EOF
            if (fgt->next_in == -1)
            {
                return AVERROR_EOF;
            }
                      ...
                      InputFilter *ifilter, AVFrame *frame)
{
    ...
    // NETINT: send hw filtered frames one by one
